...
class MasterEncoder(nn.Module):
    def __init__(self, max_seq_length, embedding_size, how_many_basic_encoders, num_atten_heads, method=1):
        super().__init__()
        self.max_seq_length = max_seq_length
        self.basic_encoder_arr = nn.ModuleList([BasicEncoder(
            max_seq_length, embedding_size, num_atten_heads, method) for _ in range(how_many_basic_encoders)])  # (A)
...
class BasicEncoder(nn.Module):
    def __init__(self, max_seq_length, embedding_size, num_atten_heads, method=1):
        super().__init__()
        self.max_seq_length = max_seq_length
        ...
        self.self_attention_layer = SelfAttention(
            max_seq_length, embedding_size, num_atten_heads, method)  # (A)
...
class SelfAttention(nn.Module):
    def __init__(self, max_seq_length, embedding_size, num_atten_heads, method=1):
        ...
        if(method==1):
            self.attention_heads_arr = nn.ModuleList([AttentionHead_Assignment(self.max_seq_length,
                                                                self.qkv_size) for _ in range(num_atten_heads)])  # (A)
        elif(method==2):
            self.attention_heads_arr = nn.ModuleList([AttentionHead_ExtraCredit(self.max_seq_length,
                                                                self.qkv_size) for _ in range(num_atten_heads)])  # (A)
...
class AttentionHead_Assignment(nn.Module):
...
class AttentionHead_ExtraCredit(nn.Module):
    def __init__(self, max_seq_length, qkv_size):
        super().__init__()
        self.qkv_size = qkv_size
        self.max_seq_length = max_seq_length
    ...
    def forward(self, sentence_portion): 
        ...
        QK_DotProduct = torch.einsum("bqn, bnk -> bqk", Q, rearrange(K, "b n e -> b e n"))
        ScaleCoeff = 1.0 / torch.sqrt(torch.tensor([self.qkv_size]).float()).to(device)
        return ScaleCoeff * torch.einsum("ban, bne -> bae", self.softmax(QK_DotProduct), V)
